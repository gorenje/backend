module Mailer
  class Client
    def send_confirm_email(details)
      send_message do
        details.merge("slug"     => "pushtech-confirmation-email",
                      "subject"  => subject_line(details,"Email Confirmation"),
                      "cc_email" => {
                        :email => "emailconfirm@pushtech.de",
                        :name  => "PushTech Email Confirm"
                      })
      end
    end
    ...
    def subject_line(details, postfix)
      "%s %s - PushTech %s" % [details["firstname"], details["lastname"], postfix]
    end
